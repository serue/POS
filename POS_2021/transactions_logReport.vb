        ...
        Try
            ...
            If cancelled_trasactions.Checked Then
                If Not txt_username.Text = "" Then
                    Using command As New SqlCommand("SELECT * FROM TRANSACTIONS WHERE TRANS_DATE BETWEEN @TRANS_DATE AND @TO AND STATUS='CANCELLED' AND CASHIER=@USERNAME ", connection)
                    ...
                    End Using
                Else
                    Using command As New SqlCommand("SELECT * FROM TRANSACTIONS WHERE TRANS_DATE BETWEEN @TRANS_DATE AND @TO AND STATUS='CANCELLED' OR CASHIER=@USERNAME ", connection)
                        command.Parameters.Add("@TRANS_DATE", SqlDbType.VarChar).Value = from_date.Text
                        command.Parameters.Add("@TO", SqlDbType.VarChar).Value = to_date.Text
                        command.Parameters.Add("@USERNAME", SqlDbType.VarChar).Value = txt_username.Text
                        Dim adapter As New SqlDataAdapter(command)
            ...
            Else
                If Not txt_username.Text = "" Then
                    Using command As New SqlCommand("SELECT * FROM TRANSACTIONS WHERE TRANS_DATE BETWEEN @TRANS_DATE AND @TO AND STATUS='COMPLETED' AND CASHIER=@USERNAME ", connection)
                        command.Parameters.Add("@TRANS_DATE", SqlDbType.VarChar).Value = from_date.Text
                        command.Parameters.Add("@TO", SqlDbType.VarChar).Value = to_date.Text
                        command.Parameters.Add("@USERNAME", SqlDbType.VarChar).Value = txt_username.Text
                        Dim adapter As New SqlDataAdapter(command)
                    ...
                    End Using
                Else
                    Using command As New SqlCommand("SELECT * FROM TRANSACTIONS WHERE TRANS_DATE BETWEEN @TRANS_DATE AND @TO AND STATUS='COMPLETED' OR CASHIER=@USERNAME ", connection)
                        command.Parameters.Add("@TRANS_DATE", SqlDbType.VarChar).Value = from_date.Text
                        command.Parameters.Add("@TO", SqlDbType.VarChar).Value = to_date.Text
                        command.Parameters.Add("@USERNAME", SqlDbType.VarChar).Value = txt_username.Text
                        Dim adapter As New SqlDataAdapter(command)
